...
class Graph(object):
    ...
    def get_adjacency_list(self):
        ...
        for node in self.nodes:
            if node.value in [edge.node_from.value for edge in node.edges]:
                adj_list[node.value] = [(edge.node_to.value, edge.value) for edge in node.edges if edge.node_from.value == node.value]
        return adj_list
    ...
    def get_adjacency_matrix(self):
        ...
        #return [[for nod in self.nodes] for node in self.nodes]
        max_index = self.get_max_index()
    ...
    def get_max_index(self):
        """return the maxinum index (value) of nodes"""
        max_index = -1
        if len(self.nodes):
            for node in self.nodes:
